...
const char *HealthImagePath = "assets/health.png";
...
int main(int argc, char const *argv[]) {
  ...
  Texture2D health_image = LoadTexture(HealthImagePath);
  ...
  if (healthLevel == 3) {
    healthLevelShopColor = GREEN;
  } else {
    healthLevelShopColor = BLACK;
  }
  ...
  Space space(SpaceImagePath, {GetScreenWidth() / 2, GetScreenHeight() - 120},
              0.3, healthLevel);
  ...
  while (!WindowShouldClose()) {
    ...
    if (state == GAME) {
      ...
      enemyManager.Update(bullets, space, IsPlaySound, meteorKillSound,
                          spaceKilledSound, 0);
      enemyManager.Draw();
      ...
      enemyManager.getEnemies().erase(
          std::remove_if(enemyManager.getEnemies().begin(),
                         enemyManager.getEnemies().end(),
                         [](Enemy &e) { return e.IsDestroyed(); }),
          enemyManager.getEnemies().end());
      bullets.erase(std::remove_if(bullets.begin(), bullets.end(),
                                   [](Bullet &b) { return b.IsDestroyed(); }),
                    bullets.end());

      DrawText(TextFormat("Score: %d", score), 10, 10, 30, WHITE);
      if (gameMode == TIMED) {
        DrawText(TextFormat("Time: %.1f", gameTimer), 10, 50, 30, WHITE);
        DrawText(TextFormat("Health: %d", space.getHealth()), 10, 90, 30,
                 WHITE);
      } else {
        DrawText(TextFormat("Health: %d", space.getHealth()), 10, 50, 30,
                 WHITE);
      }
      // EndMode2D();
    } else if (state == MENU) {
      ...
      menuManager.Update(bullets, space, IsPlaySound, meteorKillSound,
                         spaceKilledSound, 1);
      menuManager.Draw();

      DrawText("SPACE", GetScreenWidth() / 2 - 150, GetScreenHeight() / 2 - 200,
               100, WHITE);

      Rectangle playButton = {screenWidth / 2 - 100, screenHeight / 2, 200, 50};
      Rectangle shopButton = {screenWidth / 2 - 100, screenHeight / 2 + 70, 200,
                              50};
      Rectangle quitButton = {screenWidth / 2 - 100, screenHeight / 2 + 140,
                              200, 50};
      DrawButton(playButton, "Play", playButtonColor,
                 {screenWidth / 2 - 70, screenHeight / 2 + 10});
      DrawButton(shopButton, "Shop", shopButtonColor,
                 {screenWidth / 2 - 70, screenHeight / 2 + 80});
      DrawButton(quitButton, "Quit", quitButtonColor,
                 {screenWidth / 2 - 70, screenHeight / 2 + 150});
      ...
      DrawText(std::to_string(coin).c_str(), screenWidth - pos_coin.x,
               pos_coin.y, 80, WHITE);
      ...
      if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        ...
        if (mouse_pos.x >= 5 && mouse_pos.x <= 55 &&
      ...
      if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        Vector2 mouse_pos = GetMousePosition();
        if (mouse_pos.x >= 65 && mouse_pos.x <= 115 &&
            mouse_pos.y >= screenHeight - 50 && mouse_pos.y <= screenHeight) {
          if (isPlayMusic) {
            isPlayMusic = false;
          } else {
      ...
      Rectangle rec2 = {400, 100, 200, 200};
      DrawRectangleRec(rec2, healthLevelShopColor);
      DrawTexture(health_image, 390, 100, WHITE);

      std::string LevelMassage = std::to_string(speedLevel) + "/3";
      DrawText(LevelMassage.c_str(), 180, 280, 15, WHITE);

      std::string HealthLevelMassage = std::to_string(healthLevel) + "/3";
      DrawText(HealthLevelMassage.c_str(), 480, 280, 15, WHITE);
      ...
      DrawButton(backButton, "BACK", backButtonColor,
                 {screenWidth - 170, screenHeight - 50});
      ...
      std::string UpgradeMassage =
          "UPGRADE FOR " + std::to_string(cost_of_speed_powerup);

      Rectangle UpgradeButton = {400, 420, 400, 100};
      DrawButton(UpgradeButton, UpgradeMassage.c_str(), SpeedUpgradeButtonColor,
                 {420, 440});
      ...
      if (speedLevel < 3) {
        ...
        UpgradeInformation = "Your speed is full";
      }
      ...
      if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        ...
        if (CheckCollisionPointRec(mousePos, backButton)) {
          state = SHOP;
        } else if (CheckCollisionPointRec(mousePos, UpgradeButton) &&
                   coin >= cost_of_speed_powerup && speedLevel < 3) {
          coin -= cost_of_speed_powerup;
          if (IsPlaySound) {
            PlaySound(power_up);
          }
      ...
      int cost_of_health_powerup;
      if (healthLevel == 1) {
        cost_of_health_powerup = 15;
      } else if (healthLevel == 2) {
        cost_of_health_powerup = 30;
      } else if (healthLevel == 3) {
        cost_of_health_powerup = 0;
      }

      std::string UpgradeMassage =
          "UPGRADE FOR " + std::to_string(cost_of_health_powerup);

      Rectangle UpgradeButton = {400, 420, 400, 100};
      DrawButton(UpgradeButton, UpgradeMassage.c_str(), HealthUpgradeButtonColor,
                 {420, 440});

      if (healthLevel == 1) {
        HealthPower3Color = GREEN;
      } else if (healthLevel == 2) {
        HealthPower2Color = GREEN;
        HealthPower3Color = GREEN;
      } else if (healthLevel == 3) {
        HealthPower1Color = GREEN;
        HealthPower2Color = GREEN;
        HealthPower3Color = GREEN;
      }
      ...
      const char *UpgradeInformation;
      if (healthLevel < 3) {
        UpgradeInformation = "You get 3 health";
      } else {
        UpgradeInformation = "Your health is full";
      }

      DrawText(UpgradeInformation, 340, 100, 70, WHITE);

      DrawRectangleRounded(PowerRec1, 0.3, 0, HealthPower1Color);
      DrawRectangleRounded(PowerRec2, 0.3, 0, HealthPower2Color);
      DrawRectangleRounded(PowerRec3, 0.3, 0, HealthPower3Color);
      ...
      DrawTexture(health_image, 10, 10, WHITE);
      ...
      if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        ...
        if (CheckCollisionPointRec(mousePos, backButton)) {
          state = SHOP;
                   ...
                   coin >= cost_of_health_powerup && healthLevel < 3) {
          coin -= cost_of_health_powerup;
          if (IsPlaySound) {
            PlaySound(power_up);
          }
          coin_data << coin << std::endl;
          healthLevel++;
          space.setHealth(healthLevel);
          if (healthLevel == 2) {
            health_data << std::endl <<  healthLevel << std::endl;
          } else {
            health_data << healthLevel << std::endl;
          }
        }
      }
    }
